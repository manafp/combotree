...
; (function ($, window, document, undefined) {
    ...
    ComboTree.prototype.constructorFunc = function (element, options) {
        ...
        this.init();
        if (this.options.autoExpandParents) {
            this.expandParentNodesWithSelectedChildren();
        }
    }
    ComboTree.prototype.expandParentNodesWithSelectedChildren = function () {
        var _this = this;
        this._elemItemsTitle.each(function () {
            var $item = $(this);
            if ($item.find('input').prop('checked')) {
                _this.expandParents($item.parents('li.ComboTreeItemParent'));
            }
        });
    };
    ComboTree.prototype.expandParents = function ($parent) {
        if ($parent.length) {
            var $parentPlusIcon = $parent.children('span.comboTreeParentPlus').first();
            if ($parentPlusIcon.html().includes('mdi-chevron-right-circle-outline')) {
                $parentPlusIcon.html('<span class="mdi mdi-chevron-down-circle-outline"></span>');
                $parent.children('ul').first().slideDown(50);
            }
            this.expandParents($parent.parents('li.ComboTreeItemParent').first());
        }
    };
    ComboTree.prototype.init = function () {
        ...
        this._elemWrapper.append('<div id="' + this.comboTreeId + 'DropDownContainer" class="comboTreeDropDownContainer"></div><div class="comboTreeDropDownContent"></div>');

        // DORP DOWN AREA
        this._elemDropDownContainer = $('#' + this.comboTreeId + 'DropDownContainer');

        this._elemDropDownContainer.html(this.createSourceHTML());
        this._elemLoader = $('#loader');
        ...
        this.filterDropDownMenuDebounced = debounce(this.filterDropDownMenu.bind(this), 500); // 300ms debounce delay
    ...
    ComboTree.prototype.createFilterHTMLForMultiSelect = function () {
        return '<input id="' + this.comboTreeId + 'MultiFilter" type="text" class="multiplesFilter" placeholder="Type to filter"/><div id="loader" class="loader-icon loader-spin"></div>';
    }
    ...
    ComboTree.prototype.bindings = function () {
        ...
        this._elemInput.on('keyup', function (e) {
            ...
            switch (e.keyCode) {
                ...
                default:
                    if (!_this.options.isMultiple)
                        _this.filterDropDownMenuDebounced();
                    break;
            }
        });
...
ComboTree.prototype.selectMultipleItem = function (ctItem) {
    if (this.options.autoExpandParents) {
        this.expandParents($(ctItem).parents('li.ComboTreeItemParent').first());
    }
    ...
    if ($(ctItem).data("selectable") == true) {
        ...
        if (check) {
            ...
            var isChecked = $(ctItem).find("input").prop('checked');

            if (!isChecked && index) { // Uncheck
                this._selectedItems.splice(parseInt(index), 1);
                $(ctItem).find("input").prop('checked', false);
            } else if (isChecked && !index) { // Check
                this._selectedItems.push(this._selectedItem);
                $(ctItem).find("input").prop('checked', true);
            }

            // Recursively handle child items
            this.handleChildItems($(ctItem).parent('li'), isChecked);
            if (!isChecked) {
                this.checkParentSelection($(ctItem).parent('li').parent().prev('span'));
            }
        } // if check
    } // if selectable
};
 ComboTree.prototype.checkParentSelection = function ($parentSpan) {
        if (!$parentSpan.length) return;

        var $parentLi = $parentSpan.parent('li');
        var $siblingsAndSelf = $parentLi.find('> ul > li > span > input[type="checkbox"]');

        if ($siblingsAndSelf.filter(':checked').length === 0) {
            var parentData = {
                id: $parentSpan.attr("data-id"),
                title: $parentSpan.text()
            };
            var parentIndex = this.isItemInArray(parentData, this._selectedItems);

            // Remove parent from selected list if found
            if (parentIndex !== false) {
                this._selectedItems.splice(parseInt(parentIndex), 1);
                $parentSpan.find("input[type='checkbox']").prop('checked', false).prop('indeterminate', false).trigger('change');
            }

            // Recursively check upwards for the parent's parent
            this.checkParentSelection($parentLi.parent().prev('span'));
        }
    };
ComboTree.prototype.handleChildItems = function ($parentItem, isChecked) {
        var $childItems = $parentItem.find('> ul > li > span');
        $childItems.each((idx, childItem) => {
            var $child = $(childItem);
            var childData = {
                id: $child.attr("data-id"),
                title: $child.text()
            };
            var childIndex = this.isItemInArray(childData, this._selectedItems);

            // If child is already in the list, remove it
            if (childIndex) {
                this._selectedItems.splice(parseInt(childIndex), 1);
            }

            // If main item is checked, add the child to the selected list
            if (isChecked) {
                this._selectedItems.push(childData);
            }

            // Update checkbox status
            var $checkbox = $child.find("input[type='checkbox']");
            $checkbox.prop('checked', isChecked).prop('indeterminate', false).trigger('change');

            // Recursive call to handle any descendants of this child
            this.handleChildItems($child.parent(), isChecked);
    ...
    ComboTree.prototype.multiItemClick = function (ctItem) {
        ...
        var $ctItemCheckbox = $(ctItem).find('input[type="checkbox"]').first();
        var isChecked = $ctItemCheckbox.prop("checked");

        if (this.options.cascadeSelect) {
            // Handle child checkboxes
            var $childCheckboxes = $(ctItem).parent('li').find('ul input[type="checkbox"]');

            $childCheckboxes.prop('checked', isChecked).trigger('change');

            // Handle parent checkboxes
            this.updateParentCheckboxState(ctItem);
        }

        this.refreshInputVal();
    };

    ComboTree.prototype.updateParentCheckboxState = function (ctItem) {
        var $currentItem = $(ctItem).parent('li');

        while ($currentItem.length > 0) {
            var $descendantCheckboxes = $currentItem.find('input[type="checkbox"]');
            var $childCheckboxes = $currentItem.find('> ul > li > span > input[type="checkbox"]');
            var allChecked = $childCheckboxes.filter(':checked').length === $childCheckboxes.length;
            var someChecked = $descendantCheckboxes.filter(':checked').length > 0;

            // Condition to check if there's only one child and it's unchecked
            var onlyOneChildUnchecked = $childCheckboxes.length === 1 && !$childCheckboxes.filter(':checked').length;

            // Condition to check if there's only one child and it's checked
            var onlyOneChildChecked = $childCheckboxes.length === 1 && $childCheckboxes.filter(':checked').length === 1;

            var $parentCheckbox = $currentItem.children('span').find('input[type="checkbox"]');
            if ($parentCheckbox.length > 0) {
                // Only update the parent checkbox if it's not the one being clicked
                if ($parentCheckbox[0] !== $(ctItem).find('input[type="checkbox"]')[0]) {
                    if (onlyOneChildUnchecked) {
                        $parentCheckbox.prop('indeterminate', false);
                        $parentCheckbox.prop('checked', false);
                    } else if (onlyOneChildChecked) {
                        $parentCheckbox.prop('indeterminate', false);
            ...
            // Move to the next parent item
            $currentItem = $currentItem.parents('li.ComboTreeItemParent').first();
        }
    };
    ...
    ComboTree.prototype.dropDownInputKeyControl = function (step) {
        ...
        ComboTree.prototype.filterDropDownMenu = function () {
            ...
            //this._elemFilterInput.prop('disabled', true);
            this._elemLoader.show();
            if (searchText !== "") {
        ...
        this._elemLoader.hide();
        //this._elemFilterInput.prop('disabled', false);
        }
    ...
    ComboTree.prototype.processSelected = function () {
        ...
        for (let i = 0; i < this._selectedItems.length; i++) {
            let selectedItemElem = $("#" + this.comboTreeId + 'Li' + this._selectedItems[i].id);
            this.updateParentCheckboxState(selectedItemElem.find('span.comboTreeItemTitle').first());
        }
        this.refreshInputVal();
    };
    ...
    ComboTree.prototype.getSelectedIds = function () {
        ...
        // Find all checked checkboxes in the dropdown
        this._elemDropDownContainer.find('input[type="checkbox"]:checked').each(function () {
            var code = $(this).closest('li').find('span.comboTreeItemTitle').data('id');
            if (code !== undefined) {
